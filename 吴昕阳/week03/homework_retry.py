...
基于pytorch的网络编写
尝试修改NLPDemo.py做一个多分类任务，判断特定字符在字符串的第几个位置，使用rnn和交叉熵。
...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.classify = nn.RNN(input_size=sentence_length, hidden_size=5, num_layers=1, batch_first=True)  #RNN层
...
def build_sample(vocab, sentence_length):
    ...
    #指定字都未出现，则为负样本
        ...
        y = 0
    x = [index for index,word in enumerate(x) if word in set("你我他")]   #将字转换成序号，为了做embedding
    return x, y
